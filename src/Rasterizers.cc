...
// After transformation in camera space, project and plot 
// (used for point rendering) MODE 0 and MODE 1
...
void Scene::renderPoints(const Camera& eye, Screen& canvas, bool asTriangles)
{
    ...
    if (!asTriangles) {
		...
		//for(vector<Vertex>::iterator it=_objects[i]->_vertices.begin(); it!=_objects[i]->_vertices.end(); ++it) {
			...
			ProjectAndPlot( Transform(_vertices[j], eye, eye._mv), whitePixel, canvas);
		}
    } else {
		// MODE 1
		...
		for(int j=0; j<(int)_triangles.size(); j++) {
			...
			ProjectAndPlot( Transform(*_triangles[j]._vertexA, eye, eye._mv), _triangles[j]._color, canvas);
			ProjectAndPlot( Transform(*_triangles[j]._vertexB, eye, eye._mv), _triangles[j]._color, canvas);
			ProjectAndPlot( Transform(*_triangles[j]._vertexC, eye, eye._mv), _triangles[j]._color, canvas);
		}
    }
    canvas.ShowScreen();
}
...
void Scene::renderWireframe(const Camera& eye, Screen& canvas) {
    ...
    // if we use greyPixel, why are triangles drawn in cyan?
	...
	#define SCREENSPACE(xformed, xx, yy)					    \
		...
		if (agood) {
			...
			if (bgood) {
				...
				my_aalineColor(canvas._surface, ax, ay, bx, by, greyPixel); // a-b line
				if (cgood) {
					int cx,cy;
					SCREENSPACE(xformedC, cx, cy)
					my_aalineColor(canvas._surface, ax, ay, cx, cy, greyPixel); // a-c line
					my_aalineColor(canvas._surface, bx, by, cx, cy, greyPixel); // b-c line
				}
			} else {
				if (cgood) {
					int cx,cy;
...
void DrawTriangles(int iStartingTriangleIndex, int iOnePastEndingTriangleIndex) const {
	...
	for(int j=iStartingTriangleIndex; j<iOnePastEndingTriangleIndex; j++) {
		...
		coord ax,ay, bx,by, cx,cy; // typedef in Types.h
	    ...
	    if (ay<0 && by<0 && cy<0) continue; // point falls outside of canvas
	    ...
	    Filler( scene, ax, ay, bx, by, cx, cy, inCameraSpaceA, inCameraSpaceB, inCameraSpaceC,
				triangle, eye, triInfoForFillerToFill);

	    // And rasterize the triangle, interpolating per-pixel... (mode-dependent)
		canvas.RasterizeTriangle( triInfoForFillerToFill, eye, &lines[0], &left[0], &right[0]);
	} // end triangle loop
}
...
template <typename InterpolatedType>
void RenderInParallel( Scene& scene, const Camera& eye, Screen& canvas) {
    canvas.ClearScreen();
    canvas.ClearZbuffer();
...
*/
    ...
    RasterizeScene<InterpolatedType>( scene, eye, canvas).DrawTriangles(0, scene._triangles.size());
