...
int main(int argc, char *argv[]) {
    ...
    try {
		...
		coord maxi = Scene::MaxCoordAfterRescale;
		...
		auto_ptr<Light> pLight( new Light( LightDistanceFactor*maxi, LightDistanceFactor*maxi, LightDistanceFactor*maxi));
		...
		auto_ptr<Light> pLight2( new Light( LightDistanceFactor*maxi,-LightDistanceFactor*maxi,LightDistanceFactor*maxi));
		if (useTwoLights) {
		...
		Vector3 oldEyePosition(1e10,1e10,1e10), 
